...
const DialogBox = ({ setShowDialogBox }: Props) => {
    ...
    const handleSubmit = (e: any) => {
        ...
        const encodedBatch = encodeURI(inputData.batch);
        const encodedCourse = encodeURI(inputData.course);
    ...
    return (
        <div className="w-full h-full absolute backdrop-blur-lg z-10 ">
            ...
            <form
                ...
                className="sticky top-[50%] left-[50%] -translate-y-[50%] text-xl gap-4 flex items-start border-2 border-white/30 shadow-2xl bg-white/10 h-max lg:h-max rounded-lg p-8 justify-center flex-col"
            >
                <h1 className="text-blue-500 font-semibold">Edit Form</h1>
                <div>
                    <h1>Select the Course</h1>
                    <AutoComplete error="none" setCourse={handleCourseChange} />
                </div>
